...
- New `background_subtract(var, *, q=50.0, window=None, mode='diff', gap_factor=3.0)` — per-channel background removal for spectrograms, the standard first step for radio dynamic spectra (I-LOFAR, e-Callisto) and equally usable on energy spectrograms. It estimates a background per frequency channel *down the time axis*, then expresses each sample relative to it. `q` is the estimator's percentile (50 = median; 5–10 when bursts fill much of the window), `window` is `None` for one constant background over the whole window or an `int`/`timedelta`/`np.timedelta64` for a sliding one, and `mode` is `'diff'` (`S − bg`), `'ratio'` (`S / bg`) or `'db'` (`10·log10(S / bg)`). The output keeps the input's time and frequency axes and carries the right `UNITS` per mode; `ratio` and `db` yield NaN rather than ±inf where `S ≤ 0` or `bg ≤ 0`, so one bad sample can't destroy a colormap's colour scale. Also available as `dsp.arrays.background_subtract(x, y, ...)` for raw ndarrays.
- Backed by two new SciQLopPlots DSP kernels, `column_percentile` and gap-aware `rolling_percentile` — the first reductions in the module that run down the time axis per column rather than across columns within a row.
...
- Fixed the embedded JupyterLab being unrecoverable after its own File menu's **Shut Down** or **Log Out**: reopening the panel — even from Quick start — kept showing the dead page. Shut Down tears down the whole jupyverse app, and Log Out navigates to a `/logout` route no jupyverse module serves; in both cases SciQLop only raised the existing dock instead of asking jupyqt for the widget again. It now re-requests it on every open, and jupyqt >= 0.6.4 relaunches a dead server and navigates back to Lab only when the view isn't already there (so a live session keeps its state).
- Fixed SciQLop aborting when kernel code touches a widget it reached through `sciqlop_app()`. Notebook cells and agent tools both run on the jupyqt kernel thread, so `sciqlop_app().topLevelWidgets()[0].resize(...)` was a direct cross-thread Qt call — it walked the layout into the WebEngine dock and died in `QQuickWidget::createFramebufferObject`, whose GL context is main-thread affine. `sciqlop_app()` now returns the real application on the GUI thread and a `MainThreadProxy` of it anywhere else; the proxy marshals every call, attribute read and assignment to the GUI thread, wraps QObjects returned inside lists/tuples/sets/dicts (how a raw widget usually escapes), and leaves signals connectable. `sciqlop_exec_python`'s description now states that cells run off the GUI thread and that widgets must be reached through `user_api` or the namespace's `main_window`/`app`.
...
- A dev (`.dev`) build's workspace venv now installs SciQLop straight from the `main` branch instead of an unpinned `sciqlop[all]`, which silently resolved the latest *released* version from PyPI. This let a pre-release installer test run exercise stale code without any indication it wasn't testing the build under test.
