...
from .settings_delegates import get_delegate_for_field
from SciQLop.core.ui import HLine, increase_font_size
...
class SettingRow(QWidget):
    def __init__(self, field_name: str, field_info, instance, parent=None):
        ...
        layout.addWidget(self._delegate, stretch=1)
...
class SectionHeader(QWidget):
    ...
    def __init__(self, title: str, level: int = 0, parent=None):
        ...
        indent = 8 + level * 16
        layout.setContentsMargins(indent, 12, 8, 4)
        layout.setSpacing(4)
...
def _build_entry_widgets(entry_cls: type[ConfigEntry], instance: ConfigEntry,
    ...
    for field_name, field_info in entry_cls.model_fields.items():
        ...
        if isinstance(annotation, type) and issubclass(annotation, ConfigEntry):
            ...
            if level > 0:
                row.layout().setContentsMargins(16 + level * 16, 6, 16, 6)
...
class CategoryView(QWidget):
    def __init__(self, parent=None):
        ...
        self._header = QLabel()
        self._header.setContentsMargins(12, 12, 12, 12)
