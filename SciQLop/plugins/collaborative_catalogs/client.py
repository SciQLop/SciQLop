...
from PySide6.QtCore import QObject, Signal, Qt, QTimer, Slot, QThread
...
class Client(QObject):
    ...
    def login(self):
        if self.logged_in:
            return True
        creds = keyring.get_credential(self._url, None)
        if creds is None:
            return False
        if self._room_id is None:
            self._room_id = creds.username.split("@")[0]
        ...
        if cookie:
            ...
            log.info(f"Successfully logged in to {self._url}")
            return True
        log.info(f"Failed to log in to {self._url}")
        return False
