...
def test_editing_an_event_after_its_panel_is_destroyed_is_harmless(qtbot, qapp):
    """The overlay wires the provider-owned event to closures over its spans; the
    panel dying (dock closed) took the overlay down without disconnecting them,
    so the next edit of that event called into deleted objects."""
    import shiboken6
    ...
    panel._catalog_manager.add_catalog(catalog)
    (event,) = provider.events(catalog)

    shiboken6.delete(panel)
    QApplication.processEvents()

    with qtbot.captureExceptions() as exceptions:
        event.stop = event.start + timedelta(hours=3)
        event.meta_changed.emit("note")
        qtbot.wait(100)  # the range sync is debounced by a 50ms timer
