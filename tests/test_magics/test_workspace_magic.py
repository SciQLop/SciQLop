...
class TestWorkspaceCompletion:
    ...
    @patch("SciQLop.user_api.magics.completions._complete_examples")
    def test_add_example_completes_names(self, mock_examples):
        from SciQLop.user_api.magics.completions import _match_workspace
        mock_examples.return_value = ["Simple Virtual Product", "Solar Wind"]
        ctx = MagicMock()
        ctx.line_with_cursor = "%workspace add-example "
        ctx.token = ""
        result = _match_workspace(ctx)
        texts = [c.text for c in result["completions"]]
        # Names with spaces get shell-quoted by _quote_if_needed
        assert "'Simple Virtual Product'" in texts
        assert "'Solar Wind'" in texts

    @patch("SciQLop.user_api.magics.completions._complete_examples")
    def test_add_example_filters_by_prefix(self, mock_examples):
        from SciQLop.user_api.magics.completions import _match_workspace
        mock_examples.return_value = ["Solar Wind"]
        ctx = MagicMock()
        ctx.line_with_cursor = "%workspace add-example Sol"
        ctx.token = "Sol"
        result = _match_workspace(ctx)
        texts = [c.text for c in result["completions"]]
        assert "'Solar Wind'" in texts
        mock_examples.assert_called_once_with("Sol")

    def test_no_args_completion_for_status(self):
        from SciQLop.user_api.magics.completions import _match_workspace
        ctx = MagicMock()
        ctx.line_with_cursor = "%workspace status "
        ctx.token = ""
        result = _match_workspace(ctx)
        assert len(result["completions"]) == 0
